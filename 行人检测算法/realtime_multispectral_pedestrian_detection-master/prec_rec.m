function [ap, prec, tpr, fpr, thresh] = prec_rec(score, target, varargin)
...
%         [AVERAGE PRECISION, PREC, TPR, FPR, THRESH] = PREC_REC(...),
...
score_thr=0.001;
ind = logical((score>score_thr)+(target));
score = score(ind); target = target(ind);
...
prec(thresh<score_thr)=0;

ap = trapz([0; tpr], [1 ; prec]);
