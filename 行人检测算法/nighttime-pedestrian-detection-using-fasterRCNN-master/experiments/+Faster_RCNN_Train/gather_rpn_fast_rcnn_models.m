function gather_rpn_fast_rcnn_models(conf_proposal, conf_fast_rcnn, model)
    cachedir = fullfile(pwd, 'output', 'faster_rcnn_final');
    ...
    % final model들을 faster_rcnn_final dir에 복사한다.
    
    rpn_test_net_def_file=fullfile(pwd,'models','final_prototxts','proposal_test.prototxt');
    fast_rcnn_test_net_def_file=fullfile(pwd,'models','final_prototxts','detection_test.prototxt');
    feature_extractor_net_def_file=fullfile(pwd,'models','final_prototxts','feature_extractor.prototxt');
    
    copyfile(feature_extractor_net_def_file, fullfile(cachedir, 'feature_extractor.prototxt'));
    ...
    % proposal_final(RPN)과 detection_final(fast r-cnn)은 relu5까지 layer를 공유 한다.
    % detection_final : conv1 layer~pooling layer~classification layer
    % proposal_final : conv1 layer~proposal_bbox_pred layer
    % test를 위해서는, proposal_final로부터 predict_bbox를 얻고,
    % detection_final은 shared layers부분을 제거하고 pooling layer
    % 부터 region proposal을 input으로 받도록 할것
    if is_share_feature
    ...
    %proposal_detection_model.final_test=model.final_test;
