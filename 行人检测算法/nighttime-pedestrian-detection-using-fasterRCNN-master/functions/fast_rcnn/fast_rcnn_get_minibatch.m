function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = fast_rcnn_get_minibatch(conf, image_roidb, randomness, multi_frame)
...
% --------------------------------------------------------
    ...
    im_blob=[]; im_scales=[];
    for i=1:num_images
        [im, scales, ~]=my_image_blob(conf,image_roidb(i).imgs,multi_frame);
        im_blob=cat(4,im_blob,im);
        im_scales=[im_scales;scales];
    end
    ...
    for i = 1:num_images
        [im_rois, labels, bbox_targets, bbox_loss] = sample_rois(conf, image_roidb(i), fg_rois_per_image, rois_per_image, randomness);
