function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = fast_rcnn_get_valbatch(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    for i = 1:num_images
        [im_rois, labels, bbox_targets, bbox_loss] = sample_rois(conf, image_roidb(i), fg_rois_per_image, rois_per_image);
...
function [im_blob, im_scales] = get_image_blob(conf, images, random_scale_inds)
    ...
    for i = 1:num_images
        ...
        [~,name,~]=fileparts(images(i).imgs);
        tmp=strsplit(name,'_');
        n=str2num( tmp{1}(4:end) );
        if(n>=0 && n<=2 || n>=6 && n<=8) % only for set01~set03(day-time)
            im=manipulator(im);
        end
        im=myhisteq(im);
...
function [rois, labels, bbox_targets, bbox_loss_weights] = ...
    ...
    if ~isempty(fg_inds)
       ...
       fg_inds = fg_inds(1:fg_rois_per_this_image); % fixed set for validation kjh
    end
