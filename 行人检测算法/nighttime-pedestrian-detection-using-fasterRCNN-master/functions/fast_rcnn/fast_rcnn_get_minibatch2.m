function [im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = fast_rcnn_get_minibatch2(conf, image_roidb, random_sample)
...
% --------------------------------------------------------
    ...
    assert(mod(conf.batch_size2, num_images) == 0, ...
        sprintf('num_images %d must divide BATCH_SIZE %d', num_images, conf.batch_size2));
    
    rois_per_image = conf.batch_size2 / num_images;
    ...
    [~, prev.imgs, next.imgs] = adjacent_path(image_roidb.imgs, conf.skip1_img_path);
    
    [im_blob2, ~] = get_image_blob(conf, prev, random_scale_inds, level);
    if (~exist(next.imgs, 'file'))% prev frame always exist
        im_blob3=im_blob1;
    else
    ...
    bbox_targets_blob = zeros(0, 4, 'single');
    bbox_loss_blob = zeros(size(bbox_targets_blob), 'single');
    
    for i = 1:num_images
        [im_rois, labels, bbox_targets, bbox_loss] = sample_rois(conf, image_roidb(i), fg_rois_per_image, rois_per_image, random_sample);
...
function [im_blob, im_scales] = get_image_blob(conf, images, random_scale_inds, level)
    ...
    for i = 1:num_images
        im = imread(images(i).imgs);
        [set, ~] = prev_path(images(i).imgs, conf.skip1_img_path);
        if(set>=0 && set<=2 || set>=6 && set<=8) % only for day-time images
            im=manipulator(im, level);
        end
        im=myhisteq(im);
...
function [rois, labels, bbox_targets, bbox_loss_weights] = ...
    sample_rois(conf, image_roidb, fg_rois_per_image, rois_per_image, random_sample)

    overlaps = image_roidb.rois(:,5);
    rois = image_roidb.rois(:,1:4);
    bbox_targets = image_roidb.bbox_targets;
    ...
    if ~isempty(fg_inds)
        if random_sample
            ...
            fg_inds = fg_inds(1:fg_rois_per_this_image);
        end
    end
    if ~isempty(bg_inds)
        if random_sample
            ...
            bg_inds = bg_inds(1:bg_rois_per_this_image);
        end
    end
    ...
    bbox_loss_weights = [ones(length(fg_inds),4,'single'); zeros(length(bg_inds),4,'single')];
    labels = bbox_loss_weights(:,1);% bbox_loss_weights에서 1인 행은 label이 1이다.
    bbox_targets = bbox_targets(keep_inds,:);
    rois = rois(keep_inds, :);
