...
%imdb.roidb_func = pointer to the function that returns ROI
...
try
  ...
  VOCopts = get_voc_opts(root_dir,image_set);%kjh
%   VOCopts.testset = image_set;%??
  ...
  if flip
      ...
      for i = 1:length(imdb.image_ids)
          if ~exist(flip_image_at(i), 'file')
             ...
             imwrite(fliplr(im), flip_image_at(i));% JPEGImages에 flip image 저장
          end
      end
  ...
  imdb.class_to_id = ...
    containers.Map(imdb.classes, 1:imdb.num_classes);%class이름을 입력하면 class id가 나오는 함수를 생성
