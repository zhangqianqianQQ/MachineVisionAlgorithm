function conf = proposal_opt_config(varargin)
...
% --------------------------------------------------------
    ...
    ip.addParamValue('max_size',        1100,           @isscalar);
    % Images per batch, only supports ims_per_batch = 1 currently
    ip.addParamValue('ims_per_batch',   1,              @isscalar);
    % Minibatch size
    ip.addParamValue('batch_size',      120,            @isscalar);%120
    % Fraction of minibatch that is foreground labeled (class > 0)
    ip.addParamValue('fg_fraction',     0.17,           @isscalar);%0.17
    ...
    ip.addParamValue('test_max_size',   1100,           @isscalar);
    ip.addParamValue('test_nms',        0.7,            @isscalar);
