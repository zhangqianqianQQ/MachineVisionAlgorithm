function anchors = proposal_generate_anchors_opt(cache_name, varargin)
...
%% inputs
    ...
    ip.addParamValue('base_size',       20,             @isscalar);
    % ratio list of anchors
    ip.addParamValue('ratios',          2.44 ,    @ismatrix);
    % scale list of anchors
    ip.addParamValue('scales',          1.4.^[1:9],       @ismatrix);    
...
%%
    ...
    try
        ...
        base_anchor             = [1, 1, opts.base_size./opts.ratios, opts.base_size];
%         ratio_anchors           = ratio_jitter(base_anchor, opts.ratios);
        ratio_anchors          = base_anchor;
