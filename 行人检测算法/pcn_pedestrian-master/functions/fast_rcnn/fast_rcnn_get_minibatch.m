function net_inputs = fast_rcnn_get_minibatch(conf, image_roidb, hard_sample, use_lstm)
...
% --------------------------------------------------------
    ...
    if conf.batch_size > 0
        ...
        if use_lstm
            error('lstm is not support for ohem.\n');
        end
    end
    ...
    % infer timestep by feature map size
    lstm_markers_blob = ones(conf.batch_size, 16, 'single');% 
    ...
    for i = 1:num_images
        ...
        if ~isempty(hard_sample{i})
            im_rois = [im_rois; hard_sample{i}(:,1:4)];
            num_hard = size(hard_sample{i},1);
            labels = [labels; zeros(num_hard,1, 'single')];
            bbox_targets = [bbox_targets; zeros(num_hard, 4 * (num_classes+1), 'single')];
            bbox_loss = [bbox_loss; zeros(num_hard, 4 * (num_classes+1), 'single')];
            
            check_num = length(im_rois);
            try
                assert(length(labels)==check_num);
                assert(length(bbox_targets)==check_num);
                assert(length(bbox_targets)==check_num);
            catch
                fprintf('%s\n',image_roidb(i).image_path);
                pause;
            end
        end
        % Add to ROIs blob
    ...
    if use_lstm
        assert(~isempty(lstm_markers_blob));
%         lstm_markers_blob = single(permute(lstm_markers_blob, [3, 4, 2, 1]));
        net_inputs = {im_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob, lstm_markers_blob};
    else
