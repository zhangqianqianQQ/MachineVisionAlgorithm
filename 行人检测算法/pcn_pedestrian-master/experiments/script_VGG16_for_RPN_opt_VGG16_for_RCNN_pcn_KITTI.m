function script_VGG16_for_RPN_opt_VGG16_for_RCNN_pcn_KITTI()
...
% cache base
cache_base_proposal         = 'KITTI_VGG16_opt';
cache_base_pcn        = 'KITTI_VGG16_';
% train/test data
dataset                     = [];
use_flipped                 = true;
...
% save('dataset-KITTI-opt-trainval-bug', 'dataset');
ld = load('dataset-KITTI-opt-trainval');
dataset = ld.dataset;
clear ld;
%% fast rcnn
fprintf('\n***************\nfast rcnn\n***************\n');
% % stage1: train
fprintf('\n***************\nstage1\n***************\n');
Faster_RCNN_Train.do_fast_rcnn_train(conf_fast_rcnn, dataset, model.fast_rcnn.stage1, opts.do_val, 1);
% % stage2: train
fprintf('\n***************\nstage2\n***************\n');
...
%     pcn_stage2_cache_dir = fullfile(pwd, 'output', 'fast_rcnn_cachedir', model.fast_rcnn.stage2.cache_name, imdb.name);
    ...
    for i=1:length(imdb.image_ids)
       evaluate_box{i}(:, 5) = 0.8*test_box_rcnn{i}(:, 5)+0.0*test_box_rpn{i}(:,5);%+0.0*rcnn_part_lstm{i};              
       evaluate_box{i}(:, 1:4) = 0.8*test_box_rcnn{i}(:, 1:4)+0.2*test_box_rpn{i}(:,1:4);
%        evaluate_box{i}(:, 5) = test_box_rpn{i}(:,5);              
...
%            avg_part_score = mean([part_scores1_rcnn{i}(:,:);part_scores2_rcnn{i}(:,:);part_scores3_rcnn{i}], 1)';
           ...
           evaluate_box{i}(:,5) = 0.2*avg_part_score + 1.0*evaluate_box{i}(:,5);
...
%            end
                % evaluate_box
           box_temp = evaluate_box{i}(evaluate_box{i}(:,end)>0.1,:);
           for j=1:size(box_temp,1)
               x = box_temp(j,1);
               y = box_temp(j,2);
               w = box_temp(j,3) - box_temp(j,1) + 1;
