function script_VGG16_for_RPN_opt_VGG16_for_RCNN_pcn_Caltech()
...
% cache base
cache_base_proposal         = 'Caltech_VGG16_opt';
cache_base_pcn        = 'Caltech_VGG16_';
% train/test data
dataset                     = [];
use_flipped                 = true;
...
% save('dataset-Caltech-opt-trainval3', 'dataset');
ld = load('dataset-Caltech-opt-trainval3');
dataset = ld.dataset;
clear ld;
%% fast rcnn
fprintf('\n***************\nfast rcnn\n***************\n');
...
%     part_scores3_rcnn = ld.boxes_ps3;
    ...
    for i=1:length(imdb.image_ids)
       ...
       evaluate_box{i}(:, 1:4) = 0.7*test_box_rcnn{i}(:, 1:4)+0.3*test_box_rpn{i}(:,1:4);
       % evaluate_box{i}(:, 5) = 0.5*test_box_rcnn{i}(:, 5)+0.3*test_box_rpn{i}(:,5)+0.2*lstm_scores_rcnn{i}; 
       evaluate_box{i}(:, 5) = 0.5*test_box_rcnn{i}(:,5) + 0.3*test_box_rpn{i}(:,5);              
%        evaluate_box{i}(:, 1:4) = test_box_rcnn{i}(:,1:4);
