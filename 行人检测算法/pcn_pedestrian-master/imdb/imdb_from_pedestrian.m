function imdb = imdb_from_pedestrian(root_dir, image_set, set_name, flip)
...
cache_file = ['./imdb/cache/imdb_voc_' set_name '_' image_set];
if flip
...
try
  ...
  VOCopts = get_pedestrian_opts(root_dir);
  VOCopts.testset = image_set;

  imdb.name = ['voc_' set_name '_' image_set];
  ...
  imdb.eval_func = @imdb_eval_pedestrian;
  imdb.roidb_func = @roidb_from_pedestrian;
  imdb.image_at = @(i) ...
