function roidb = roidb_from_pedestrian(imdb, varargin)
...
try
  ...
  if ~imdb.flip
      for i = 1:length(imdb.image_ids)
        ...
        try
          voc_rec = Pedreadrecord(sprintf(VOCopts.annopath, imdb.image_ids{i}), imdb.sizes(i,:));
        catch
          voc_rec = [];
        end
        if ~isempty(regions)
        ...
        if 0
            ...
            figure(1),imshow(im);
            hold on;
            boxes_temp = roidb.rois(i).boxes;
            for kk=1:size(boxes_temp,1);
            x = boxes_temp(kk,1);
            y = boxes_temp(kk,2);
            w = boxes_temp(kk,3) - boxes_temp(kk,1) + 1;
            h = boxes_temp(kk,4) - boxes_temp(kk,2) + 1;
      ...
      for i = 1:length(imdb.image_ids)/2
        ...
        try
          voc_rec = Pedreadrecord(sprintf(VOCopts.annopath, imdb.image_ids{i*2-1}), imdb.sizes(i*2-1,:));
        catch
          voc_rec = [];
        end
        if ~isempty(regions)
        ...
        if 0
            ...
            figure(1),imshow(im);
            hold on;
            boxes_temp = roidb.rois(i*2-1).boxes;
            for kk=1:size(boxes_temp,1);
            x = boxes_temp(kk,1);
            y = boxes_temp(kk,2);
            w = boxes_temp(kk,3) - boxes_temp(kk,1) + 1;
            ...
            figure(2),imshow(im);
            hold on;
            boxes_temp = roidb.rois(i*2).boxes;
            for kk=1:size(boxes_temp,1);
            x = boxes_temp(kk,1);
            y = boxes_temp(kk,2);
            w = boxes_temp(kk,3) - boxes_temp(kk,1) + 1;
...
%     gt_overlap = rec.overlap(:, gt_classes(i));
%     all_overlap = boxoverlap(all_boxes, gt_boxes(i, :));
%     rec.overlap(:, gt_classes(i)) = all_overlap;
end
