function imo = cnn_ucf101_get_im_diff_batch(images, varargin)
...
if strcmp(opts.frameSample, 'random')
    opts.nFramesPerVid = 1;
end
...
%     end
    ...
    %read rgb and gray img
    if strcmp(opts.frameSample, 'random')
        file = dir(fullfile(imgDir,vid_name));
        file = file(3:end);
        start = randi(numel(file));
    
        frames_rgb{i,1,1} = [vid_name file(start).name] ;
    
        frames{i,1,1} = [vid_name file(start).name] ;
   
    else
    file = dir(fullfile(imgDir,vid_name));
    file = file(3:end);
    % read with alpha image
%     start = randi((numel(file)-opts.nFramesPerVid));
    % read only view point image
    start = randi((numel(file)-opts.nFramesPerVid+1));
    for k = 1:opts.nFramesPerVid
        frames_rgb{i,1,k} = [vid_name file(k+start-1).name] ;
    end
    for k = 1:opts.nFramesPerVid
        frames{i,1,k} = [vid_name file(k+start-1).name] ;
    end
    end
%     %change this part to read img
%     for k = 1:opts.nFramesPerVid
