function stats = cnn_resnet_train_dag(net, imdb, getBatch, varargin)
...
modeltxtPath = fullfile(opts.expDir,'net-train.txt');
...
fclose(fid);
  if ~isempty(values_loss)
    ...
    subplot(1,2,2) ; plot(1:epoch, values','o-') ; ylim([0 0.5])
...
%     moreopts.nFramesPerVid = 5;
    moreopts.nFramesPerVid = opts.nFramesPerVid;
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    net.meta.curNumFrames = repmat(1 / numel(inputs{4}),1,numel(net.layers)); % nFrames = instances/labels
    ...
    if opts.prefetch
      ...
      if ~isempty(nextBatch)
        moreopts.frameList = state.getBatch(state.imdb, nextBatch) ;
      else 
