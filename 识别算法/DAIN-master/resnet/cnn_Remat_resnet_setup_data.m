function imdb = cnn_Remat_resnet_setup_data(varargin)
...
[Training_set, Testing_set, cats]= feval(sprintf('get_resnet_%s_split', lower(opts.dataSet)),opts.nSplit,opts.splitDir);
...
for v=1:numel(vids)
  vid_name = vids(v); vid_name = vid_name{1};
  ims = dir(fullfile(imdb.imageDir,  vid_name, '*.jpg'));
  imdb.images.nFrames(v) = numel(ims); 
end

function [train_fnames,test_fnames, saction]= get_resnet_remat_split(isplit,splitdir)
  
  fid = fopen([splitdir '/classInd.txt']);

  classes =      textscan(fid, '%s');
  saction = classes{1}(2:2:end);
  fname = sprintf('%s/patch2000testlist0%d.txt',splitdir,isplit);
  fid = fopen(fname);
  ...
  fclose(fid);
     itr = 1;
  fname = sprintf('%s/patch2000trainlist0%d.txt',splitdir,isplit);
  fid = fopen(fname);
  while 1
   tline = fgetl(fid);
   if tline==-1
