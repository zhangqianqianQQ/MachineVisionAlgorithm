...
% function [N b] = computeNormals(X, Y, Z, superpixels, R)
% Clip out a 2R+1 x 2R+1 window at each point and estimate the normal from points within this window. In case the window straddles more than a single superpixel, only take points in the same superpixel as the centre pixel. Takes only a few seconds.
% Input: 
%	X, Y, Z are the point cloud entries in centimetres
%	superpixels
%	R: L\infinity radius for points to consider in the 2D image.
% Output: 
% The normal at pixel (x,y) is N(x, y, :)'pt + b(x,y) = 0

	N = NaN([size(Z), 3]);
	Xf = fillHoles(X);  %Only used for finding the orientation of the normal
	Yf = fillHoles(Y);
	Zf = fillHoles(Z);
