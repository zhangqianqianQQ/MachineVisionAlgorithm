function imdb = my_extract_patches(sample, ratio_trainning)
    ...
    % Init 
    imdb.images.data = im2single(zeros(27, 27, total_size));
...
%         plot(location_oth.detection(:, 1), location_oth.detection(:, 2), 's', 'MarkerSize',10, 'Color', 'g');
        ...
        for j=1:size(location_epi.detection, 1)
            ...
            if size(subImage, 1) == 27 && size(subImage, 2) == 27
                ...
                imdb.images.data(:, :, curr_size + set_size) = rgb2gray(im2single(subImage));
