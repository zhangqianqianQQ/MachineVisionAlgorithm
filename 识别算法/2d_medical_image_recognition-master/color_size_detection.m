...
for i = 1:sample 
    ...
    img = im2single(imread(['Detection/img' num2str(i) '/img' num2str(i) '.bmp']));
    ...
    % Get blue
    blue_color= img(:,:,3) ./ (img(:,:,1)+img(:,:,2)+img(:,:,3));
    blue_color=blue_color./max(max(blue_color));
    mask = blue_color < 0.66;
