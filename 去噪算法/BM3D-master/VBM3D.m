...
elseif length(size(Xnoisy)) == 3% the input argument is a 3D video (saptio-temporal) matrix
...
%%%% If the original video exists, then extract it to 'Xorig' --- it is 
%%%% needed if one wants to compute PSNR and ISNR
if exist('Xorig') == 1,
    randn('seed', 0);
    if ischar(Xorig) == 0,
...
if dump_information == 1,
    ...
    text_vid = 'Denoised';
    if (isCharacterName == 1),
        text_vid = Xnoisy(1:end-4);
    end
