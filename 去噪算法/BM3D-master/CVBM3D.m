...
if exist('sigma', 'var') ~= 1,
    Xnoisy = 'SMg20.avi';  sigma = 20;  Xorig = 'SM.avi';
end
...
if exist('Xorig', 'var') == 1,
    if ischar(Xorig) == 1;    
        ...
        for cf = 1:NumberOfFrames
            Xorig(:,:,:,cf) = single(mo(cf).cdata(:,:,:))/255;
        end
        clear mo;
...
if nargout == 1
    ...
    for cf = 1:NumberOfFrames
        Xdenoised(:,:,:,cf) = single(mo(cf).cdata(:,:,:))/255;
    end
    clear  mo    
end
...
if dump_information == 1
    ...
    PSNR_TEXT='';
    if exist('Xorig', 'var') == 1
        PSNR = 10*log10(1/mean2((Xorig-Xdenoised).^2));
