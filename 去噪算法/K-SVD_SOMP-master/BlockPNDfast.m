function [output0 Dictionary] =BlockPNDfast(input,second_input,hW0,hD0,Dictionary,matrix_sigma)
...
%     end
% end
% % inputtemp=im2col(input,[9 9],'sliding');
% % residual=inputtemp-Dictionary* cof_matrix;
% % dct_res=dct2(residual);
% % mcoef=dct_res(1,1);
% % [num ind]=sort(dct_res(:),1,'ascend');
% % for i1=1:48
% %     if dct_res(ind(i1)) ~= 0
% %         dct_res(ind(i1))=0;
% %         continue 
...
%         blocks=tempsimblocks;
        CoefMatrix =  SOMPerr(Dictionary,tempsimblocks, errorGoal,BETA,8 );
