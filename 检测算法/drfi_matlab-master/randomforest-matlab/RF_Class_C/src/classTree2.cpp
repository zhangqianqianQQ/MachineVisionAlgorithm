...
void predictClassTree(double *x, int n, int mdim, int *nrnodes, int *treemap,
						  ...
						  for (i = 0; i < n; ++i) {
							  ...
							  while (nodestatus[k] != NODE_TERMINAL) {
								  ...
								  if (cat[m] == 1) {
									  ...
									  // Rprintf( "\t***x: %.3f\tsplit: %.3f\tbestvar: %.d\n", x[m + i * mdim], xbestsplit[k], m);
									  k = (x[m + i * mdim] <= xbestsplit[k]) ?
										  treemap[k] - 1 : treemap[k + *nrnodes] - 1;
									  // Rprintf( "k: %d\n", k+1 );
								  } else {
