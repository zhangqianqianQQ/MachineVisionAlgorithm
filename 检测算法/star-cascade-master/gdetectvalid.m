function [dets, boxes, info] = gdetectvalid(pyra, model, thresh, bbox, overlap, valid)

% See: ../gdetect.m
% This is a modified version of gdetect.m that takes an argument 'valid' that specifies
% the root location and component that must be used for the detection.
%
% valid      require the detection to use a particular location & component
...
if valid.c > 0
  % invalidate all components other than valid.c by setting
  % their offset to -inf
  for i = 1:length(model.rules{model.start})
    if i ~= valid.c
      model.rules{model.start}(i).offset.w = -inf;
    end
  end
end
...
for s = L
  ...
  model = symbol_score(model, s, latent, pyra, bbox, overlap, valid);
end
...
function model = symbol_score(model, s, latent, pyra, bbox, overlap, valid)
% model  object model
% s      grammar symbol
...
if latent && s == model.start
  ...
  for i = 1:length(model.rules{model.start})
    ...
    for level = model.interval+1:length(model.rules{model.start}(i).score)
      if valid.c > 0
        % eliminate all root placements other than the valid one
        if level ~= valid.l
