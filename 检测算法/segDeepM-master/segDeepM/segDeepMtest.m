function res = segDeepMtest(rcnn_model, imdb, suffix)
...
config = feat_opts.config;
config.seg.saveFeat = 0; % do not save feature in test 
        ...
        for i = folds{f}
            ...
            if config.ctx.use
                dc = rcnn_load_cached_pool5_features(config.ctx.cache_name, ...
                    imdb.name, image_ids{i});
                dc.feat = rcnn_pool5_to_fcX(dc.feat, feat_opts.layer, rcnn_model.ctx);
                dc.feat = rcnn_scale_features(dc.feat, feat_opts.feat_norm_mean_ctx);
                d.feat = cat(2,d.feat,dc.feat);
            end
            
            if config.seg.use
                detwin = d.boxes;
                feats = segCachePyramidFeat(imdb.image_ids{i},detwin, config);
            end
            
            zs = bsxfun(@plus, d.feat*rcnn_model.detectors(f).W(1:config.seg.segFeatPivot,:), rcnn_model.detectors(f).B);
            ...
            for j = 1:num_classes
                ...
                if config.seg.use
                    ...
                    [sz,~]=segGetScore(rcnn_model.detectors.W(sp:end,j),j,feats,detwin,config);
                    z = z + sz;
                end
