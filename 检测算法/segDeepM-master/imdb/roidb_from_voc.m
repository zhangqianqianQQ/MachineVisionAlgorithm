...
try
    ...
    for i = 1:length(imdb.image_ids)
        ...
        imsize = size(I);
        roidb.rois(i).ctx = roidb.rois(i).boxes;
        bw = ( roidb.rois(i).boxes(:,3) - roidb.rois(i).boxes(:,1) + 1 )/2;
        bh = ( roidb.rois(i).boxes(:,4) - roidb.rois(i).boxes(:,2) + 1 )/2;
        
        roidb.rois(i).ctx(:,1) = max(1,round(roidb.rois(i).ctx(:,1) - bw));
        roidb.rois(i).ctx(:,2) = max(1,round(roidb.rois(i).ctx(:,2) - bh));
        roidb.rois(i).ctx(:,3) = min(imsize(2), round(roidb.rois(i).ctx(:,3) + bw));
        roidb.rois(i).ctx(:,4) = min(imsize(1), round(roidb.rois(i).ctx(:,4) + bh));
    end
